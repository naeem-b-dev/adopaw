...
function extractLikelyChatUserIdFromJWT(jwt) {
  try {
    const [, b64] = (jwt || "").split(".");
    if (!b64) return "";
    const json = JSON.parse(global.atob ? atob(b64) : Buffer.from(b64, "base64").toString("utf8"));
...
export default function MessageBubble({ item, currentUserId }) {
  ...
  const tsColor = (palette?.neutral && palette.neutral[500]) || theme.colors.outline;
...
const styles = StyleSheet.create({
  ...
  bubble: { paddingVertical: 8, paddingHorizontal: 12, maxWidth: "85%" },
  bubbleMine: {
    borderTopLeftRadius: R, borderTopRightRadius: R,
    borderBottomLeftRadius: R, borderBottomRightRadius: 6,
  },
  bubbleOther: {
    borderTopLeftRadius: R, borderTopRightRadius: R,
    borderBottomLeftRadius: 6, borderBottomRightRadius: R,
  },
